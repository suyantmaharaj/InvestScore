...
const SCRIPT = [
  ...
  {
    act: 'Act 2',
    title: 'The PM Experience (4 minutes)',
    tagline: 'Show Lerato making an investment decision – and seeing the full portfolio analytics.',
    steps: [
      {
        label: 'Log in as PM',
      ...
      },
      {
        ...
        timing: '3:45 – 4:15',
        note: 'Click Medium next to SDG 6. Click Save targets. Banner SME portal will now show this target.',
      },
      {
        label: 'Show the Employment dashboard',
        screen: 'Navigate to Employment (/employment)',
        dialogue: 'Employment is the primary impact lens for Sanlam. This page breaks down the 850 jobs across the portfolio by youth, gender, race, and permanency – the exact data structure of the 104+ Impact Report, now live and filterable.',
        timing: '4:15 – 4:45',
        note: 'Point to the youth employment bar and Black workforce percentage. These map directly to SDG 8 KPIs.',
      },
      {
        label: 'Show Trends and Compare tools',
        screen: 'Navigate to Trends (/trends), then briefly Compare (/compare)',
        dialogue: 'Trends shows Lerato whether the portfolio is moving in the right direction year-over-year. Compare lets her put any two companies side-by-side across all 17 SDGs – the kind of benchmarking that used to take a full analyst day in Excel.',
        timing: '4:45 – 5:15',
        note: 'On Trends, point to SDG 8 trajectory. On Compare, load Wakanda Capital vs Banner Green Tech to show the contrast.',
      },
    ],
  },
  {
    act: 'Act 3',
    title: 'The SME Experience (3 minutes)',
    tagline: 'Show Sipho submitting data, understanding his scores, and learning.',
    steps: [
      {
      ...
      {
        label: 'Show the Submit form',
        screen: 'Navigate to Submit (/submit)',
        dialogue: "This is how Sipho submits his quarterly data. Employment numbers, energy usage, B-BBEE certificate, CSI spend. Every field has in-line guidance. He doesn't need a consultant to fill this in. The moment he hits Submit, his SDG score recalculates and Lerato's portfolio view updates.",
        timing: '6:30 – 7:00',
        note: 'Walk through 2–3 sections of the form – Employment, then Environmental. Point to the help text on each field. Do not actually submit.',
      },
      {
        label: 'Show Chase',
      ...
      {
        label: 'Show the Learning Hub',
        screen: 'Navigate to Learning (/learning)',
        dialogue: 'And for SMEs who need more context – the Learning Hub. Video lessons on every SDG, quizzes, certificates. Chase can link directly to a lesson. A first-generation business owner in Soweto can watch a 5-minute video on B-BBEE and understand exactly what they need to submit. No consultant. No jargon.',
        timing: '7:45 – 8:15',
        note: 'Open one lesson – e.g. SDG 1 or B-BBEE. Play the first few seconds of the video. Show the quiz questions beneath.',
      },
    ],
  },
  {
    act: 'Act 4',
    title: 'Admin & Governance (1 minute)',
    ...
    steps: [
      {
        ...
        dialogue: 'The Admin portal gives Koko full control – company management, B-BBEE certificate verification, and a complete audit log. Every action on the platform is timestamped and traceable.',
        timing: '8:15 – 8:45',
        note: 'Show the Companies page (/companies) first – scroll the company list.',
      },
      {
        label: 'Show the Audit Log',
        screen: 'Navigate to Audit Log (/audit)',
        dialogue: "Every submission, every score change, every PM target – logged here with a timestamp, user ID, and the exact data changed. This is the CRISA and UNPRI-ready governance layer. Sanlam's compliance team can pull a complete data lineage for any company at any time.",
        timing: '8:45 – 9:15',
        note: 'Scroll the audit log – point to a submission entry and a target-set entry. Show the timestamp and actor columns.',
      },
    ],
  },
  {
    ...
    steps: [
      {
        ...
        screen: 'No screen – face the judges',
        dialogue: 'Three things make INvestScore different from any other ESG platform in South Africa. First: it is built specifically for the 104+ methodology – not adapted from a generic ESG tool. Second: it serves both sides of the relationship – the PM making investment decisions and the SME understanding and improving. Third: Chase. An AI coach that knows each company\'s data, speaks plain English to a first-generation business owner in Soweto, and can read their documents. No other impact investment platform in SA has this.',
        timing: '9:15 – 10:15',
      },
      {
      ...
      },
      {
        ...
        dialogue: "We're happy to demo any specific feature. The platform has a full submission flow, Learning Hub with video lessons, employment and trends dashboards, company comparison tools, and a complete audit log – all built directly from the 104+ Impact Report structure.",
        timing: '10:45 – 12:00',
      },
    ],
  },
...
export default function DemoScriptPage() {
  ...
  const [view,       setView]       = useState<'script' | 'checklist' | 'credentials' | 'questions'>('script');
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') window.location.href = '/login';
    };
  ...
  return (
    <div style={{ fontFamily: 'system-ui, sans-serif', background: '#0a0f1a', minHeight: '100vh', color: 'white', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Top nav */}
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '12px 20px', borderBottom: '1px solid rgba(255,255,255,0.08)', background: BRAND.navy }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '4px' }}>
          {(['script', 'checklist', 'credentials', 'questions'] as const).map(v => (
          ...
          <a href="/login"
            style={{ padding: '6px 12px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.12)', cursor: 'pointer', fontSize: '12px',
              color: 'rgba(255,255,255,0.4)', textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
            ← Login
          </a>
        </div>
      </div>
          ...
          <div style={{ width: '220px', flexShrink: 0, borderRight: '1px solid rgba(255,255,255,0.08)', padding: '16px 12px', overflowY: 'auto' }}>
            ...
            {SCRIPT.map((a, ai) => (
              <div key={ai}>
                ...
                {ai === activeAct && a.steps.map((st, si) => (
      ...
      {/* CHECKLIST VIEW */}
      {view === 'checklist' && (
        <div style={{ flex: 1, padding: '32px', maxWidth: '700px', margin: '0 auto', width: '100%', overflowY: 'auto' }}>
